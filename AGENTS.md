...
- Provider-agnostic boundary: `app/lib/identity.ts` exposes `Viewer`
  ...
  Concrete adapters are private to that module, so all viewer resolution goes
  through configured selection. `header` reads trusted
  `oai-authenticated-user-*` values; `local` reads synthetic `x-local-*`
  development headers without a network dependency.
- Selection via `IDENTITY_PROVIDER` (or `AUTH_PROVIDER`) env var:
  `header` / `oai` / `chatgpt` → header adapter, `local` / `dev` →
  local adapter, **default → `deny` (refuses every inbound identity header)**.
  The local adapter additionally requires the compile-time capability emitted
  only by Vite's `serve` command. Every production build hard-codes that
  capability off; selecting `local` then throws instead of authenticating or
  falling back. Trusting either header scheme without its intended runtime
  boundary permits impersonation.
