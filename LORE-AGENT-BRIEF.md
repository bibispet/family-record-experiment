# Lore (frx) — Standing Agent Brief · v3.8
...
cookie is still assembled in `tests/identity.test.ts` via
`serializeLocalIdentityCookie` and handed to the built worker — no test has yet
driven an actual `/dev/sign-in` POST against a live dev server and followed the
Set-Cookie through.)
...
Merged. Evidence: `git merge-base --is-ancestor 6fbccb0 origin/main` exits 0 —
`wip/agent1-flag-reconciliation`'s tip is an ancestor of `main`, so it reached
the trunk via the hardening → dev-signin-a → main merge chain (`dfddde5`,
`0567a12`).


**CLOSED — P2 · archive the last two unpreserved things.**
Both archived on origin. Evidence: `git branch -avv` lists
...
**CLOSED — P1 · commit the two blocker fixes.**
Shipped on the hardening branch and now in `main`. Evidence: `a83d555`
(hardening: deny-by-default dev routes, build-elimination, deployment contract)
and `c183f47` (hardening: build-time dev route guards, trusted-proxy check); both
are ancestors of `origin/main` (`git merge-base --is-ancestor` exits 0).


**CLOSED — P2 · stale comment.**
Fixed. Evidence: `ff7e522` ("test: correct stale DEV_MODE comment; note greps
are a canary in elimination test") is an ancestor of `origin/main`.


**CLOSED — P1 · typecheck and lint on `hardening/rung2-dev-routes`.**
Fixed in three commits, all now in `main`. Evidence: `ccde57e` (mechanical —
`vite/client` types on the dev routes + `withEnvAsync` typings), `e363828`
(the `postLocalSignIn` deletion, alone, with body + zero-reference grep in the
message), `f6bd83d` (wire typecheck + lint into `npm test`). All three gates
pass on the merged result.
...
**CLOSED — P2 · record the local-adapter decision in `DECISIONS.md`.**
Done. Evidence: `910e144` ("docs: record local adapter ships runtime-gated,
blocks deploy (rung 7)") is an ancestor of `origin/main`, and the entry
"2026-09-01 — Local identity adapter ships runtime-gated, not build-eliminated"
is present in `DECISIONS.md`.


**CLOSED — P3 · merge `dev-signin-a` into `main`.**
Done with `--no-ff` so it is a single reverrible unit. Evidence: `0567a12`
(`git show --oneline 0567a12` → "Merge branch 'dev-signin-a'") is an ancestor
of `origin/main`; revert handle `git revert -m 1 0567a12` recorded in
`DECISIONS.md`. The hardening → dev-signin-a leg is `dfddde5`, also recorded.
